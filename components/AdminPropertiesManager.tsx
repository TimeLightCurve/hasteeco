      ...
      <section className="mt-5 overflow-hidden rounded-3xl border border-stone-200 bg-white">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-right text-sm">
            <thead className="bg-stone-50 text-base text-stone-400"><tr><th className="px-5 py-4">ملک</th><th className="px-5 py-4">شناسه</th><th className="px-5 py-4">نوع</th><th className="px-5 py-4">وضعیت</th><th className="px-5 py-4">زیربنا</th><th className="px-5 py-4">قیمت</th><th className="px-5 py-4">عملیات</th></tr></thead>
            <tbody className="divide-y divide-stone-100">
              {filtered.map((property) => (
                <tr key={property.listingId} className="group hover:bg-stone-50">
                  <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="relative h-12 w-16 overflow-hidden rounded-lg bg-stone-100"><Image src={property.images[0]} alt="" fill sizes="64px" className="object-cover" /></div><div><strong className="block max-w-56 truncate">{property.title}</strong><span className="text-base text-stone-400">{property.location.city}</span></div></div></td>
                  <td className="px-5 py-4 text-stone-400">#{property.listingId}</td>
                  <td className="px-5 py-4">{property.propertyType}</td>
                  <td className="px-5 py-4"><span className={`rounded-full px-3 py-1 text-base font-bold ${property.status === "for-sale" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{property.status === "for-sale" ? "فروش" : "اجاره"}</span></td>
...
function FormSection({ title, description, children }: { title: string; description: string; children: React.ReactNode }) { return <section className="space-y-4 rounded-2xl border border-stone-200 bg-white p-5 sm:p-6"><div className="border-b border-stone-100 pb-4"><h3 className="font-black">{title}</h3><p className="mt-1 text-base text-stone-400">{description}</p></div>{children}</section> }
function Field(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) { const { label, style, ...inputProps } = props; return <label className="block"><span className="mb-1.5 block text-base font-bold text-stone-500">{label}</span><input {...inputProps} style={formControlStyle(inputProps.dir, style)} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3.5 py-3 text-sm outline-none focus:border-brand-green focus:bg-white focus:ring-3 focus:ring-brand-green/10" /></label> }
function TextArea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }) { const { label, style, ...inputProps } = props; return <label className="block"><span className="mb-1.5 block text-base font-bold text-stone-500">{label}</span><textarea {...inputProps} style={formControlStyle(inputProps.dir, style)} className="w-full resize-y rounded-xl border border-stone-200 bg-stone-50 px-3.5 py-3 text-sm leading-6 outline-none focus:border-brand-green focus:bg-white focus:ring-3 focus:ring-brand-green/10" /></label> }
function SelectField({ name, label, defaultValue, options }: { name: string; label: string; defaultValue: string; options: string[] }) { return <label className="block"><span className="mb-1.5 block text-base font-bold text-stone-500">{label}</span><select name={name} defaultValue={defaultValue} className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3.5 py-3 text-sm outline-none focus:border-brand-green">{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label> }
