  ...
  return (
    <div className="p-5 sm:p-8 lg:p-10">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div><p className="text-base font-bold tracking-[0.16em] text-brand-gold">نمای کلی</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">داشبورد مدیریت</h1><p className="mt-2 text-sm text-stone-400">وضعیت لحظه‌ای مجموعه املاک هستیکو</p></div>
      ...
      <section className="mt-10 overflow-hidden rounded-3xl border border-stone-200 bg-white">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-right text-sm"><thead className="bg-stone-50 text-base text-stone-400"><tr><th className="px-6 py-4">ملک</th><th className="px-6 py-4">شناسه</th><th className="px-6 py-4">وضعیت</th><th className="px-6 py-4">شهر</th><th className="px-6 py-4">قیمت</th></tr></thead><tbody className="divide-y divide-stone-100">{properties.slice(0, 6).map((property) => <tr key={property.listingId} className="hover:bg-stone-50"><td className="px-6 py-4 font-bold">{property.title}</td><td className="px-6 py-4 text-stone-400">#{property.listingId}</td><td className="px-6 py-4"><span className="rounded-full bg-brand-green/10 px-3 py-1 text-base font-bold text-brand-green">{property.status === "for-sale" ? "فروش" : "اجاره"}</span></td><td className="px-6 py-4">{property.location.city}</td><td className="px-6 py-4 font-bold">{number.format(property.price.amount)} تومان</td></tr>)}</tbody></table></div>
      </section>
    </div>
  )
}
...
function Metric({ label, value, note, color }: { label: string; value: string; note: string; color: string }) {
  return <article className="rounded-3xl border border-stone-200 bg-white p-6"><div className={`mb-8 h-2 w-10 rounded-full ${color}`} /><p className="text-base font-bold text-stone-400">{label}</p><strong className="mt-2 block text-4xl font-black">{value}</strong><p className="mt-3 text-base text-stone-400">{note}</p></article>
}
